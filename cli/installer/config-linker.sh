#!/bin/bash
# ~/.config/nordwolf/linker.sh

NORDWOLF="$HOME/.config/nordwolf/config"
CONFIG="$HOME/.config"

mapfile -t DIRS < <(find "$NORDWOLF" -mindepth 1 -maxdepth 1 -type d -printf '%f\n')
# ── Helpers ───────────────────────────────────────────────────────────────────
...
_add() {
    local app="$1"
    local force="${2:-}"
    local src="$NORDWOLF/$app"
    local dst="$CONFIG/$app"

    if [[ ! -d "$src" ]]; then
        error "$app: not found in nordwolf/config — skipping"
        return
    fi
    ...
    if [[ -L "$dst" && -e "$dst" ]]; then
        ...
        if [[ "$force" == "-f" ]]; then
            if [[ -d "$src" ]]; then
                ...
                error "$app: source also missing in nordwolf/config"
            fi
        fi
    fi
}

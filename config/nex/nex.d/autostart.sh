...
BSPDIR="$HOME/.config/nex"
export PATH="${PATH}:$BSPDIR/src"

# ── Source all config atoms ───────────────────────────────────────────────────
source "$BSPDIR/nex.d/sources/services.sh"
source "$BSPDIR/nex.d/sources/wallpaper.sh"
source "$BSPDIR/nex.d/sources/widget.sh"
...
killall -9 xsettingsd sxhkd dunst ksuperkey xfce4-power-manager \
    nexl polybar quickshell qs snapserver picom tor vicinae plank pomod

# ── Polkit agent ──────────────────────────────────────────────────────────────
[[ ! $(pidof xfce-polkit) ]] && /usr/lib/xfce-polkit/xfce-polkit &

# ── Keybindings ───────────────────────────────────────────────────────────────
sxhkd -c "$BSPDIR/nex.d/sxhkdrc" &
...
# ── Picom ─────────────────────────────────────────────────────────────────────
[[ $is_svc_picom == true ]] && "$BSPDIR/nex.d/picomrc" &
...
[[ $is_widget == true ]] && bash ~/.config/nex/widgets/polybar/launch.sh &

##---------------------------------------- Manager Scripts : All those use nexl subscribe to bring about some action----------------------------
